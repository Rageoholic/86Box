...
#include "vid_rivatnt.h"

#define BIOS_RIVATNT_PATH		L"roms/video/nvidia/NV4_STB_velocity.rom"
...
rivatnt_mmio_read(uint32_t addr, void *p)
{
	...
	if ((addr >= 0x110000) && (addr <= 0x11ffff)) return rivatnt->bios_rom.rom[addr & rivatnt->bios_rom.mask];
...
rivatnt_mmio_read_w(uint32_t addr, void *p)
{
	...
	if ((addr >= 0x110000) && (addr <= 0x11ffff)) return ((uint16_t *) rivatnt->bios_rom.rom)[(addr & rivatnt->bios_rom.mask) >> 1];
...
*rivatnt_init(const device_t *info)
{
	...
	rom_init(&rivatnt->bios_rom, romfn, 0xc0000, 0x8000, 0x7fff, 0, MEM_MAPPING_EXTERNAL);

	svga_init(&rivatnt->svga, rivatnt, rivatnt->vram_size,
